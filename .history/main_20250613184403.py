...
def check_title_page(playwright:Playwright, url:str, result:str, browser_names:list[str]):
    with sync_playwright() as playwright:
        ...
        for browser_name in browser_names:
            ...
            if title == result:
                sucses_test+=1
                result_test+=f"✅  Проверка номер {steps} пройдена, браузер - {browser_type.name}, ожидаемый результат {result} свопадает с  фактическим {title}\n"
            else:
                fail_test+=1
                result_test+=f"❌ Провека номер {steps} не пройдена, браузер - {browser_type.name}, ожидаемый результат {result} НЕ соответсвует фактическому {title}\n"
        
        result_test = f"Отчет по теситрвоанию:\n{result_test}\nИтоги тестирования:\nУспешных тестов {sucses_test}/{steps}\nНеуспешных тестов {fail_test}/{steps}" 
